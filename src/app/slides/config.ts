...
export const SLIDES: SlideInfo[] = [
  ...
  // ─── ACT 2: "What could go wrong" ────────────────────────────
  {
    ...
    notes: [
      ...
      "",
      "POINT to the crash comment: \"Now imagine the server restarts right there. The card was charged. The restaurant was never told. No undo. No record.\"",
      "",
  ...
  // ─── ACT 4: "The small things that matter" ───────────────────
  {
    ...
    number: 7,
    notes: [
      "SAY: \"Here's a detail that matters. Every step gets a unique, deterministic ID — stepId.\"",
      "",
      "POINT to the idempotencyKey line: \"Pass it to Stripe. If the step retries three times, Stripe sees the same key and only charges once.\"",
      "",
      "SAY: \"You get this for free. No UUIDs. No database.\"",
      "",
      "TRANSITION: Press →.",
    ].join("\n"),
  },
  {
    ...
    number: 8,
    notes: [
      "SAY: \"Not all errors are the same. Some mean 'this will never work' — that's a FatalError. Stop trying. Start undoing.\"",
      "",
      "SAY: \"Others mean 'the service is just busy' — that's a RetryableError. Wait, try again.\"",
      "",
      "POINT to the retry timeline: \"Each retry waits longer. Exponential backoff — the runtime handles it.\"",
      "",
      "TRANSITION: Press →.",
    ].join("\n"),
  },
  ...
  },
  {
    ...
    notes: [
      ...
      "",
      "TRANSITION: Press d → arm failure at assignDriver. Run an order. Watch the compensations appear in the fuchsia section. Then return.",
    ].join("\n"),
  },
  ...
  // ─── ACT 9: "Go build" ───────────────────────────────────────
  {
    ...
    number: 23,
    notes: [
      "SAY: \"That's the Vercel Workflow SDK. Two words — 'use workflow' and 'use step'. Everything else follows from there.\"",
      "",
      "POINT to each pill: \"Hooks for waiting. Streaming for real-time. Sagas for cleanup.\"",
      "",
      "SAY: \"It's GA tonight. vercel.com/docs/workflow. Ship it tonight.\"",
      "",
      "PAUSE for applause.",
      "",
