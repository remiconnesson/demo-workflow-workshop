...
export default function TimeoutRaceSlide() {
  const demoTimeout = slideScenarios.timeoutRace.input.driverTimeout ?? "2m";
    ...
    <div className="flex h-full w-full items-center justify-center gap-16 px-20">
      ...
      <div className="flex-1 max-w-2xl">
        ...
        <pre className="mt-8 rounded-2xl border border-amber-500/20 bg-zinc-950 p-8 font-mono text-2xl leading-relaxed overflow-hidden">
          <Cmt>{`// driver has ${demoTimeout} for this slide demo`}</Cmt>{"\n"}
          ...
          {"  "}<Punc>{"})"}</Punc><Punc>)</Punc>,{"\n"}
          {"  "}<Fn>sleep</Fn><Punc>(</Punc><Str>{JSON.stringify(demoTimeout)}</Str><Punc>)</Punc>.<Fn>then</Fn><Punc>(</Punc><Punc>()</Punc> <Punc>=&gt;</Punc> <Punc>({"{"}</Punc>{"\n"}
        ...
        <p className="mt-6 text-xl text-zinc-400">
          If the driver doesn&apos;t accept in {demoTimeout}, the workflow
          times out and triggers compensation.
        </p>
      </div>
